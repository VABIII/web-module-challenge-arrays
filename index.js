...
function copy(arr_name){
 const array_2 = [...arr_name];
  return array_2;
}
...
function is31Flavors(arr_name){
 if (arr_name.length === 31) {
   return true; 
  }
  else {return false;}
}
...
function addFlavor(arr_name, flavor){
  arr_name.splice(0, 0,flavor);
  return arr_name;
}
...
function removeLastFlavor(arr_name){
  arr_name.pop();
  return arr_name;
}
...
function getFlavorByIndex(arr_name, num){
  return arr_name[num];
}
